...
function renderRow(flight: Flight) {
    return (
        <>
            ...
            <LinkTableCell text={flight.destination.name} to={`/api/aircraft/${flight.destination.id}`}/>
            <LinkTableCell text={flight.aircraft.registration} to={`/api/aircraft/${flight.aircraft.id}`}/>
...
export const FlightsTable: FunctionComponent = () => {
    return <Table fetchItems={Api.flights.list}
                  renderRow={renderRow}
                  headers={headers}
    />;
};
